/** \file	FileOperations.cpp
            ...
            E-mail: subj@mail.ru\n
            Description: Implementation of FileOperation class and CFileExeption class.
*/
...
//************************************************************************************************************
FExeption::FExeption(DWORD dwErrCode)
{
    LPVOID lpMsgBuf;
...
FExeption::FExeption(CString sErrText)
{
    m_sError = sErrText;
    m_dwError = 0;
}
...
void FileOperation::DoDelete(CString sPathName)
{
    CFileFind ff;
    ...
    if (CheckPath(sPath) == PATH_IS_FILE)
    {
        ...
        if (!DeleteFile(sPath)) throw new FExeption(GetLastError());
        return;
    }
    ...
    ff.Close();
    if (!RemoveDirectory(sPathName) && !m_bAborted) throw new FExeption(GetLastError());
}


void FileOperation::DoFolderCopy(CString sSourceFolder, CString sDestFolder, bool bDelteAfterCopy)
{
    CFileFind ff;
    ...
    while (bRes)
    {
        ...
        if (ff.IsDirectory()) // source is a folder
        {
            ...
            if (CheckPath(sPathDest) == PATH_NOT_FOUND)
            {
                ...
                {
                    ff.Close();
                    throw new FExeption(GetLastError());
                }
            }
...
bool FileOperation::Delete(CString sPathName)
{
    try
    {
        DoDelete(sPathName);
...
bool FileOperation::Rename(CString sSource, CString sDest)
{
    try
    {
...
void FileOperation::DoRename(CString sSource, CString sDest)
{
    if (!MoveFile(sSource, sDest)) throw new FExeption(GetLastError());
}


void FileOperation::DoCopy(CString sSource, CString sDest, bool bDelteAfterCopy)
{
    ...
    {
        throw new FExeption(_T("Wrong operation"));
    }
    // folder to folder
...
void FileOperation::DoFileCopy(CString sSourceFile, CString sDestFile, bool bDelteAfterCopy)
{
    ...
    if (!CopyFile(sSourceFile, sDestFile, bOvrwriteFails)) throw new FExeption(GetLastError());
    if (bDelteAfterCopy)
    {
...
bool FileOperation::Copy(CString sSource, CString sDest)
{
...
bool FileOperation::Replace(CString sSource, CString sDest)
{
    if (CheckSelfCopy(sSource, sDest)) return true;
    bool bRes;
    try
...
CString FileOperation::ChangeFileName(CString sFileName)
{
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                int iCount = _tstoi(sCount.GetString());
                iCount++;
...
bool FileOperation::IsFileExist(CString sPathName)
{
    HANDLE hFile;
...
int FileOperation::CheckPath(CString sPath)
{
...
void FileOperation::PreparePath(CString &sPath)
{
    if (sPath.Right(1) != "\\") sPath += "\\";
}


bool FileOperation::CanDelete(CString sPathName)
{
...
CString FileOperation::ParseFolderName(CString sPathName)
{
...
void FileOperation::CheckSelfRecursion(CString sSource, CString sDest)
{
...
bool FileOperation::CheckSelfCopy(CString sSource, CString sDest)
{
    bool bRes = false;
...
void FileOperation::AddExcludePath(const CString& strPath)
{
    m_vecExcludeFolder.push_back(strPath);
}

void FileOperation::ClearExcludePath()
{
    m_vecExcludeFolder.clear();
}

BOOL FileOperation::IsExcludePath(const CString& strPath)
{
    BOOL bExclude = FALSE;
    for (int ii = 0; ii < (int)m_vecExcludeFolder.size(); ii++)
    {
        CString strExclude = m_vecExcludeFolder[ii];
        if (strExclude == strPath.Left(strExclude.GetLength()))
        {
            bExclude = TRUE;
            break;
        }

...
class QueueSharedMemory::QueueSharedMemoryImpl
{
private:
#pragma pack(push, 1)
...
public:
    ...
    bool HasQueueInfo()
    {
        return m_queue_info ? true : false;
    }
...
QueueSharedMemory::QueueSharedMemory()
    : m_impl(std::make_unique<QueueSharedMemoryImpl>())
{
...
bool QueueSharedMemory::HasQueueInfo()
{
    return m_impl->HasQueueInfo();
}

int QueueSharedMemory::Initialize(const std::string& name, uint32_t queue_size)
{
    return m_impl->Initialize(name, queue_size);
}

void QueueSharedMemory::Finalize()
{
    m_impl->Finalize();
}

std::string QueueSharedMemory::GetName() const
{
    return m_impl->GetName();
}

int QueueSharedMemory::GetInformation(uint8_t* buffer)
{
    return m_impl->GetInformation(buffer);
}

int QueueSharedMemory::SetInformation(uint8_t* buffer)
{
    return m_impl->SetInformation(buffer);
}

int QueueSharedMemory::Clear()
{
    return m_impl->Clear();
}

int QueueSharedMemory::Push(uint8_t* buffer, uint32_t buffer_len)
{
    return m_impl->Push(buffer, buffer_len);
}

int QueueSharedMemory::Front(uint8_t* buffer, uint32_t buffer_len)
{
    return m_impl->Front(buffer, buffer_len);
}

int QueueSharedMemory::Pop()
{
    return m_impl->Pop();
}

int QueueSharedMemory::SetData(uint32_t pos, uint8_t* buffer, uint32_t buffer_len)
{
    return m_impl->SetData(pos, buffer, buffer_len);
}

int QueueSharedMemory::GetData(uint32_t pos, uint8_t* buffer, uint32_t buffer_len)
{
    return m_impl->GetData(pos, buffer, buffer_len);
}

uint32_t QueueSharedMemory::GetUseSize() const
{
    return m_impl->GetUseSize();
}

uint32_t QueueSharedMemory::GetQueueSize() const
{
    return m_impl->GetQueueSize();
}

uint32_t QueueSharedMemory::GetFreeSize() const
{
    return m_impl->GetFreeSize();
}

uint32_t QueueSharedMemory::GetWinErrorCode() const
{
...
int TestQueueSharedMemory()
{
    ...
    // 초기화
    QueueSharedMemory queue1;
    if (queue1.Initialize(name, 128))
        return 1;

    QueueSharedMemory queue2;

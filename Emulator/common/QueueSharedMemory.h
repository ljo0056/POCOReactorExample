...
///  @brief   프로세스간 공유 할수 있는 공유메모리 원형 Queue 를 생성 한다.
...
{
private:
    class QueueSharedMemoryImpl;
    std::unique_ptr<QueueSharedMemoryImpl> m_impl;
...
public:
    ...
    ///                    name 설정 시 Global namespace("Global\\" 접두어) 로 명시적으로 만들고자 한다면 관리자 권한이 필요 하다.
    ///                    자세한 내용은 MSDN 을 참조
